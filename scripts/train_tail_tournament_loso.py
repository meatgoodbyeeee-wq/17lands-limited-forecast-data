...
def pairs(d,fs,tail=.15,gap=.35,cap=6000,seed=20260922,set_weights=None):
 ...
 for _,g in d.groupby("set"):
  ...
  sw=1.0 if set_weights is None else float(set_weights.get(str(g.iloc[0]["set"]),1.0))
  for i,j,gp,nt in cand:
   z=X[i]-X[j];lab=int(y[i]>y[j]);xx += [z,-z];yy += [lab,1-lab];ww += [gp*(1+nt)*sw]*2
 return np.asarray(xx),np.asarray(yy),np.asarray(ww)
def fit(d,fs,set_weights=None):
 X,y,w=pairs(d,fs,set_weights=set_weights);m=HistGradientBoostingClassifier(learning_rate=.05,max_iter=180,max_leaf_nodes=15,l2_regularization=1,random_state=20260922);m.fit(X,y,sample_weight=w);return m
def score(m,d,fs):
...
def main():
 ...
 # Mild recency weighting: newer development sets receive more pairwise weight.
 chrono=["KHM","STX","AFR","MID","VOW","NEO","SNC","DMU","BRO","ONE","MOM","LTR","WOE","LCI","MKM","OTJ","MH3","BLB","DSK","FDN","DFT","TDM"]
 set_weights={s:0.65+0.35*i/(len(chrono)-1) for i,s in enumerate(chrono)}
 for hold in sorted(d["set"].unique()):
  tr=d.set!=hold;te=d.set==hold;m=fit(d.loc[tr],fs,set_weights);s=score(m,d.loc[te],fs);tmp=d.loc[te].copy();tmp["rank"]=s
 ...
 d["tail_pred"]=pred;out={"fin_used":False,"scoring":"pairwise_tournament_rank_blend_15pct_recency_weighted","folds":folds,"overall":met(d,"tail_pred")};z.out.parent.mkdir(parents=True,exist_ok=True);z.out.write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
